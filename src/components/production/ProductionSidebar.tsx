...
import { BarChart3, Trophy, Settings, X, ChevronLeft, ChevronRight, Flame, Award, Wrench, ClipboardCheck, HardHat } from "lucide-react";
import { NavLink, useLocation } from "react-router-dom";
import { cn } from "@/lib/utils";

const navItems = [
  { icon: BarChart3, label: "My Stats", path: "/production/stats" },
  { icon: ClipboardCheck, label: "Onboarding", path: "/onboarding" },
  { icon: Trophy, label: "Leaderboard", path: "/production/leaderboard" },
  { icon: Wrench, label: "Tools", path: "/production/tools" },
  { icon: Flame, label: "The Pit", path: "/production/pit" },
  { icon: Award, label: "Points History", path: "/production/points-history" },
  { icon: Settings, label: "Settings", path: "/production/settings" },
];
...
export function ProductionSidebar({ mobileOpen = false, onMobileClose }: ProductionSidebarProps) {
    ...
    <>
      ...
      >
        <div className="flex items-center justify-between p-3 border-b border-primary-foreground/10">
          {!collapsed && (
            <span className="text-sm font-heading uppercase tracking-wide flex items-center gap-2">
