...
interface RoleTimeClockWidgetProps {
  role: 'user' | 'supplementer' | 'office';
  onShiftChange?: () => void;
}

export function RoleTimeClockWidget({ role, onShiftChange }: RoleTimeClockWidgetProps) {
  const { user } = useAuth();
  ...
  const fetchShifts = useCallback(async () => {
    ...
    const { data: active } = await (supabase.from("role_shifts" as any) as any)
      .select("*")
      .eq("user_id", user.id)
    ...
    const { data: last } = await (supabase.from("role_shifts" as any) as any)
      .select("*")
      .eq("user_id", user.id)
      .eq("role", role)
      .eq("status", "completed")
  ...
  const performClockIn = async (loc: { lat: number; lng: number } | null, outOfZone = false) => {
    ...
    try {
      ...
      const { data: existingShift } = await (supabase.from("role_shifts" as any) as any)
        .select('id, role')
        .eq('user_id', user.id)
        .is('clock_out_at', null)
        .maybeSingle();
      ...
      const { data, error } = await (supabase.from("role_shifts" as any) as any)
        .insert({
          user_id: user.id,
          role: role,
  ...
  const handleClockOut = async () => {
    ...
    try {
      ...
      const { error } = await (supabase.from("role_shifts" as any) as any)
        .update({
      ...
      <DialogContent>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="roleNotes">Notes (optional)</Label>
            <Textarea
              id="roleNotes"
              value={notes}
