...
Deno.serve(async (req) => {
  ...
  try {
    const { leadId, customerName, quoteAmount, serviceType, repName, leadSource } = await req.json();
    ...
    // Also get the assigned rep's email
    ...
    if (lead?.assigned_to) {
      const { data: repUser } = await supabaseAdmin.auth.admin.getUserById(lead.assigned_to);
      if (repUser?.user?.email && !recipientEmails.includes(repUser.user.email)) {
        recipientEmails.push(repUser.user.email);
      }
    }
...
<body style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; color: #333;">
  <div style="background: #000; padding: 24px; text-align: center; border-radius: 8px 8px 0 0;">
    <h1 style="color: #fff; font-size: 24px; margin: 0;">🎉 Deal Won!</h1>
    <p style="color: #c91f5e; font-size: 14px; margin: 8px 0 0;">Next Generation Roofing</p>
  </div>
  <div style="border: 1px solid #eee; border-top: none; padding: 24px; border-radius: 0 0 8px 8px;">
    <p style="font-size: 16px; margin: 0 0 16px;">
      <strong>${repName || "A team member"}</strong> just closed a deal!
    </p>
    ...
    <div style="text-align: center; margin: 24px 0;">
      <a href="${leadUrl}" style="display: inline-block; background: #22c55e; color: #fff; padding: 14px 40px; border-radius: 6px; text-decoration: none; font-weight: bold; font-size: 16px;">
        VIEW LEAD
      </a>
    </div>
    ...
    <p style="font-size: 12px; color: #999; text-align: center;">
      Next Generation Roofing • Deal Won Notification
    </p>
  </div>
</body>
</html>`;
    ...
    const res = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        to: recipientEmails,
        subject: `🎉 Deal Won — ${customerName} — ${formattedAmount}`,
        html,
      }),
    });
    ...
    console.error("notify-deal-won error:", error);
